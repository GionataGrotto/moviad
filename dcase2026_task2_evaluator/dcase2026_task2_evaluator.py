    ...
    for machine_idx, target_machine in enumerate(machines):
        ...
        for section_id in section_ids:
            ...
            if not os.path.exists(anomaly_score_path) or \
                ...
                # Load DCASE2026 baseline results
                anomaly_score_path = "{dir}/anomaly_score_DCASE2026T2{machine}_{section}_test_seed{seed}{tag}_Eval.csv".format(
                    dir=target_dir,
                    machine=target_machine,
                    section=section_id,
                    seed=seed,
                    tag=tag)
                decision_result_path = "{dir}/decision_result_DCASE2026T2{machine}_{section}_test_seed{seed}{tag}_Eval.csv".format(
                    dir=target_dir,
                    machine=target_machine,
                    section=section_id,
                    seed=seed,
            ...
            if not os.path.exists(anomaly_score_path) or \
                ...
                print(f"\tanomaly_score_path :\t{os.path.exists(anomaly_score_path)}\t{anomaly_score_path=}")
                print(f"\tdecision_result_path :\t{os.path.exists(decision_result_path)}\t{decision_result_path=}")
                print(f"\tground_truth_path :\t{os.path.exists(ground_truth_path)}\t{ground_truth_path=}")
                print(f"\tgt_domain_path :\t{os.path.exists(gt_domain_path)}\t{gt_domain_path=}")
                print(f"\tgt_attribute_path :\t{os.path.exists(gt_attribute_path)}\t{gt_attribute_path=}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ground_truth_root", type=str, default=".",
                        help="Root directory containing ground_truth_* and teams directories")

    args = parser.parse_args()
    
    ground_truth_root = Path(args.ground_truth_root)
    os.chdir(ground_truth_root)
    ...
    # The upstream release targets the five-machine evaluation set.  Keep the
    # same evaluator logic usable for the seven-machine development set by
    # deriving the machine list and aggregate column schemas from the local
    # ground truth files.
    MACHINE_TYPE_LIST[:] = machine_types
    OFFICIAL_SCORE_COLUMNS[:] = [
        "official score", "arithmetic mean", "harmonic mean (source)",
